...
function pathToRoute(opts, memo, path) {
  ...
  if ((0, _fs.statSync)(absPath).isDirectory()) {
    // 相对目录用于生成路由
    const dir = (0, _path.join)(parentDir, path); // 查找目录下的 layout 作为本级目录路由，注意 index 在 normalizePath 处理

    const pageFile = (0, _utils.findFile)({
      base: absPath,
      pattern: 'layout',
      type: 'javascript'
    }); // 递归查找子目录
    ...
    const routePath = (0, _utils.genRoutePath)(dir);
    ...
    if (pageFile) {
      route.componentPath = pageFile.path; // 目录本身不存在实体路由，单纯作为 parent path
    } else {
    ...
    memo.push(addImport(route, opts)); // 文件
  } else {
    const bName = (0, _path.basename)(path, (0, _path.extname)(path));
    const routePath = (0, _utils.genRoutePath)((0, _path.join)(parentDir, bName));
    const routeName = (0, _lodash.camelCase)(routePath);
    memo.push(addImport({
      path: routePath,
      name: routeName,
...
function addImport(route, opts) {
  ...
  if (path) {
    try {
      let componentPath = (0, _utils.winPath)((0, _path.relative)((0, _path.join)(opts.root, '..'), path)); // typescript 不允许 import .ts|tsx
...
function getRoutes(opts) {
  const root = opts.root,
        _opts$parentDir2 = opts.parentDir,
        parentDir = _opts$parentDir2 === void 0 ? '' : _opts$parentDir2;
  const pagePaths = findPages((0, _path.join)(root, parentDir));
