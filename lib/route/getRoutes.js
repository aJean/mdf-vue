...
function pathToRoute(opts, memo, path) {
  ...
  if ((0, _fs.statSync)(absPath).isDirectory()) {
    ...
    if (layout) {
      route.componentPath = layout.path; // 目录本身不存在实体路由，单纯作为 parent path
    } else {
    ...
    if (childName !== 'layout') {
      const routePath = (0, _utils.genRoutePath)((0, _path.join)(parentDir, childName));
      const routeName = (0, _lodash.camelCase)(routePath);
      const childRoute = {
        path: routePath,
        name: routeName,
...
function addImport(route, opts) {
  ...
  if (path) {
    try {
      let componentPath = (0, _utils.winPath)((0, _path.relative)(opts.root, path)); // typescript 不允许 import .ts|tsx
